...
def macrotrends_client_name() -> str:
    proxy_url = get_config_value("MACROTRENDS_PROXY_URL")
    if proxy_url:
        return "configured Macrotrends proxy"
...
def fetch_macrotrends_via_proxy(url: str, log: Optional[List[str]] = None) -> Optional[str]:
    proxy_url = get_config_value("MACROTRENDS_PROXY_URL")
    if not proxy_url:
        return None

    token = get_config_value("MACROTRENDS_PROXY_TOKEN")
    headers = {}
    if token:
        headers["Authorization"] = f"Bearer {token}"
        headers["X-Macrotrends-Proxy-Token"] = token

    try:
        if "{url}" in proxy_url:
            target_url = proxy_url.replace("{url}", requests.utils.quote(url, safe=""))
            response = requests.get(target_url, headers=headers, timeout=60)
        else:
            response = requests.get(proxy_url, params={"url": url}, headers=headers, timeout=60)
        if response.ok and response.text:
            if log is not None:
                log.append(f"Fetched via Macrotrends proxy: {url}")
            return response.text
        if log is not None:
            log.append(f"Macrotrends proxy returned {response.status_code} for {url}: {response.text[:200]}")
    except Exception as exc:
        if log is not None:
            log.append(f"Macrotrends proxy request failed for {url}: {exc}")
    return None
...
def make_request_with_retry(
    ...
    if use_proxy and get_config_value("MACROTRENDS_PROXY_URL"):
        return fetch_macrotrends_via_proxy(url, log=log)
...
def render_data_loader() -> None:
    ...
    if get_config_value("MACROTRENDS_PROXY_URL"):
        st.sidebar.success("Macrotrends proxy is configured.")
    else:
        st.sidebar.warning("No Macrotrends proxy configured. Hosted fetches may be blocked.")
    ...
    if not st.session_state.data:
        if macrotrends_was_blocked():
            st.warning(
                ...
                "This means the deployment host/network is blocked by Macrotrends."
            )
            st.markdown(
                """
                **Fix for hosted deployments**

                This hosted app is not using a Macrotrends proxy yet. Set these Streamlit secrets:
                ...
                Then run the proxy from a machine/network where Macrotrends works:
                ...
                The proxy still fetches Macrotrends statement pages; it only changes the network used to retrieve the HTML.
