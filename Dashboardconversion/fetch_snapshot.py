...
def fetch_snapshot(tickers: list[str], delay: float) -> tuple[dict, list[str]]:
    headers = {
    ...
    for idx, ticker in enumerate(tickers, start=1):
        ...
        if any(isinstance(data.get(statement), pd.DataFrame) and not data[statement].empty for statement in STATEMENT_TYPES):
            out[ticker] = data
        else:
...
def main() -> None:
    ...
    parser.add_argument("--delay", type=float, default=1.5, help="Delay between Macrotrends requests.")
    args = parser.parse_args()

    data, logs = fetch_snapshot(parse_tickers(args.tickers), args.delay)
