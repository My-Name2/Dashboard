...
except ImportError:  # pragma: no cover - requests fallback remains available
    curl_requests = None
...
def make_request_with_retry(
    ...
    curl_session = curl_requests.Session(impersonate="chrome124") if curl_requests is not None else None
    for attempt in range(max_retries):
        try:
            actual_headers = dict(headers)
            actual_headers["User-Agent"] = random.choice(USER_AGENTS)
            if curl_session is not None:
                response = curl_session.get(url, headers=actual_headers, timeout=20, allow_redirects=True)
            else:
            ...
            if response.ok:
                if response.history and "?freq=Q" in url and "?freq=Q" not in response.url:
                    ...
                    if log is not None:
                        log.append(f"Redirected; retrying quarterly URL: {corrected_url}")
                    time.sleep(random.uniform(2, 4))
                    if curl_session is not None:
                        corrected = curl_session.get(corrected_url, headers=actual_headers, timeout=20)
                    else:
                        corrected = session.get(corrected_url, headers=actual_headers, timeout=20)
                    if corrected.ok:
            ...
            if response.status_code == 403:
                if log is not None:
                    client_name = "curl_cffi browser impersonation" if curl_session is not None else "requests"
                    log.append(
                        f"403 for {url}; Macrotrends blocked this request using {client_name}. "
                        "Retrying, but cloud hosts may remain blocked."
                    )
        ...
        if attempt < max_retries - 1:
            time.sleep(min(base_backoff_seconds * (2**attempt), 30) + random.uniform(0.5, 1.5))
    return None
...
def fetch_employee_count_data(
    ticker: str, company_slug: str, headers: Dict[str, str], log: Optional[List[str]] = None
) -> Optional[pd.DataFrame]:
    url = f"https://www.macrotrends.net/stocks/charts/{ticker.upper()}/{company_slug}/number-of-employees"
    html = make_request_with_retry(url, headers, log=log)
    if not html:
        return None
...
def fetch_ticker_data(tickers: List[str], polite_delay: float = 1.5) -> FetchResult:
    headers = {
    ...
    for i, ticker in enumerate(tickers, start=1):
        ...
        for statement_title, statement_slug in STATEMENT_TYPES.items():
            ...
            html = make_request_with_retry(url, headers, log=logs)
            if html:
        ...
        emp_df = fetch_employee_count_data(ticker, slug, headers, log=logs)
        ...
        has_macrotrends_statement = any(
            isinstance(data.get(statement), pd.DataFrame) and not data[statement].empty
            for statement in STATEMENT_TYPES
        )
        if has_macrotrends_statement:
            out[ticker] = data
        else:
...
def render_data_loader() -> None:
    ...
    polite_delay = st.sidebar.number_input("Delay between Macrotrends requests", 0.0, 30.0, 1.5, 0.5)
    col_a, col_b = st.sidebar.columns(2)
    if col_a.button("Fetch", type="primary", use_container_width=True):
        result = fetch_ticker_data(tickers, polite_delay=polite_delay)
...
def render_summary_tab() -> None:
    if not st.session_state.data:
        if any("403 for https://www.macrotrends.net" in line for line in st.session_state.logs):
            st.warning(
                ...
                "Try running locally, increasing the request delay, or loading a snapshot fetched from a network "
                "that Macrotrends allows."
            )
        else:
